...
import { DocumentService, DocumentAnalysable, AnalyseResultat, HistoriqueEntry } from '../../../../core/services/document';
...
export class AnalyseDocumentComponent implements OnInit {
  ...
  voirDocument(doc: DocumentAnalysable): void {
    if (!doc.url) return;
  ...
  chargerHistorique(docId: number): void {
    this.historiqueLoading.set(true);
    this.docService.getHistoriqueDocument(docId).subscribe({
      next:  (h) => { this.historique.set(h); this.historiqueLoading.set(false); },
      error: () => { this.historique.set([]); this.historiqueLoading.set(false); },
    });
  }

  historiqueDetail(entry: HistoriqueEntry): string {
    const d = entry.details ?? {};
    if (entry.action === 'REJETER' && d['motif']) return d['motif'];
    if (entry.action === 'ANALYSE' && d['plagiat_score'] !== undefined)
      return `Plagiat ${d['plagiat_score']}% — IA ${d['ia_score']}%`;
    if (entry.action === 'UPLOAD' && d['version']) return `Version ${d['version']}`;
    return '';
  }
  ...
  validerDocument(): void {
    ...
    this.docService.decisionDocument(r.document_id, 'valider', '').subscribe({
      next: () => {
        ...
        this.chargerHistorique(r.document_id);
      },
      error: (err) => {
