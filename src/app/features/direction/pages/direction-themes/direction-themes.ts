...
import { DossierExamen, ExamenService } from '../../../../core/services/examen/examen';
...
export class DirectionThemesComponent implements OnInit {
  ...
  dossiersValides  = signal<DossierExamen[]>([]);
  dossiersRejetes  = signal<DossierExamen[]>([]);
  dossiersLoading  = signal(false);
  ...
  // ── Modal rejet de thème ──────────────────────────────────────────────────
  showRejetModal    = signal(false);
  rejetThemeId      = signal<number>(0);
  rejetMotif        = signal('');
  rejetEtudiantNom  = signal('');
  isRejeting        = false;

  // ── Modal nouvelle session ────────────────────────────────────────────────
  showSessionModal = false;
  sessionForm = { numero: '1', label: '', date_debut: '', date_fin: '' };
  sessionSuccessMsg = signal('');
  sessionErrorMsg   = signal('');

  // ── Modal correction thème ───────────────────────────────────────────────
  showCorrectionModal = signal(false);
  correctionThemeId   = signal(0);
  correctionMotif     = signal('');

  // ── Confirm clôture soutenance ───────────────────────────────────────────
  clotureSoutenanceId  = signal<number | null>(null);
  clotureSoutenanceNom = signal('');
  ...
  afficherVue(v: DirectionVue): void {
    this.vue.set(v);
    if (v === 'dashboard') { this.chargerThemes(); this.chargerStatsSession(); this.chargerDossiers(); }
  ...
  chargerDossiers(): void {
    ...
    this.examenService.getDossiersExamen().subscribe({
      next: (data) => {
        // Documents validés (après vérification par le service examen)
        this.dossiersValides.set(data.filter(d => d.statut === 'VALIDATED'));
        // Documents rejetés (frais, IA, plagiat…)
        this.dossiersRejetes.set(data.filter(d => d.statut === 'REJECTED'));
        this.dossiersLoading.set(false);
      },
      error: () => this.dossiersLoading.set(false),
    });
  }
  ...
  ouvrirRejetModal(id: number, etudiantNom: string): void {
    this.rejetThemeId.set(id);
    this.rejetEtudiantNom.set(etudiantNom);
    this.rejetMotif.set('');
  ...
  confirmerRejet(): void {
    ...
    this.themeService
      .updateTheme(this.rejetThemeId(), { statut: 'REJECTED', remarques_examinateur: motif })
      .subscribe({
        next: () => {
          this.isRejeting = false;
          this.fermerRejetModal();
          this.chargerThemes();
          this.toast.success('Thème rejeté — l\'étudiant a été notifié.');
        },
        error: () => {
          this.isRejeting = false;
          this.toast.error('Erreur lors du rejet du thème.');
        },
      });
  }

  demanderCorrection(id: number): void {
    this.correctionThemeId.set(id);
    this.correctionMotif.set('');
    this.showCorrectionModal.set(true);
  }

  confirmerCorrection(): void {
    const motif = this.correctionMotif().trim();
    if (!motif) return;
    this.themeService
      .updateTheme(this.correctionThemeId(), { statut: 'REJECTED', remarques_examinateur: motif })
      .subscribe(() => {
  ...
  ouvrirModal(etudiant: EtudiantSansSoutenance): void {
    if (!etudiant.est_eligible) {
      this.toast.error(`${etudiant.prenom} ${etudiant.nom} n'est pas encore éligible (UE ou frais non validés).`);
      return;
    }
  ...
  cloturerSoutenance(id: number, etudiant: string): void {
    this.clotureSoutenanceId.set(id);
    this.clotureSoutenanceNom.set(etudiant);
  }

  confirmerCloture(): void {
    const id = this.clotureSoutenanceId();
    if (id === null) return;
    this.soutenanceService.cloturerSoutenance(id).subscribe({
      next: () => { this.clotureSoutenanceId.set(null); this.chargerEvaluations(); },
      error: (err) => {
        this.clotureSoutenanceId.set(null);
        this.toast.error(err?.error?.detail ?? 'Erreur lors de la clôture.');
      },
    });
  }
  ...
  confirmerNouvelleSession(): void {
    ...
    if (!date_debut || !date_fin) {
      this.sessionErrorMsg.set('Veuillez saisir les dates de début et de fin de session.');
      return;
    }
    this.sessionErrorMsg.set('');
    this.sessionSuccessMsg.set(
