...
export class AffectationsComponent implements OnInit {
  ...
  soumettrReattribution(affId: number): void {
    if (!this.nouveauDirecteurId) {
      this.errorMsg.set('Veuillez sélectionner un nouveau directeur.'); return;
    }
    if (!this.motifReattribution.trim()) {
      this.errorMsg.set('Veuillez saisir le motif de réattribution.'); return;
    }
    ...
    this.affectationService.reattribuer(affId, this.nouveauDirecteurId, this.motifReattribution).subscribe({
      next: () => {
        this.successMsg.set('Directeur réattribué avec succès. L\'étudiant et le nouveau directeur ont été notifiés.');
        this.fermerReattribution();
        this.charger();
        this.actionInProgress.set(null);
      },
      error: (err) => {
        this.errorMsg.set(err?.error?.detail ?? 'Erreur lors de la réattribution.');
        this.actionInProgress.set(null);
      },
    });
  }
