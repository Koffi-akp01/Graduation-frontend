...
export class RattrapagesComponent implements OnInit {
  ...
  filteredRattrapages = computed(() => {
    const q = this.searchTerm().toLowerCase();
    return this.rattrapages().filter(r =>
      !q || r.etudiant_nom.toLowerCase().includes(q) || r.ue_code.toLowerCase().includes(q)
    );
  });
  ...
  creer(): void {
    if (!this.form.etudiant || !this.form.ue || !this.form.date) {
      this.errorMsg.set('Étudiant, UE et date sont obligatoires.');
      return;
    }
    this.rattrapageService.creer(this.form).subscribe({
      next: (r) => {
        this.rattrapages.update(list => [r, ...list]);
        this.successMsg.set('Rattrapage créé et étudiant notifié.');
